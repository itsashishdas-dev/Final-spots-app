...
import { Search, Map as MapIcon, X, Terminal, Database, ChevronRight, Activity, Zap, Mountain, Clock } from 'lucide-react';
...
const GridView: React.FC = () => {
  ...
  const { spots, isStale, lastUpdated } = useSpotsData();
    ...
    <div className="h-full flex flex-col bg-[#020202] pt-safe-top font-mono relative">
       ...
       <div className="max-w-5xl mx-auto w-full h-full flex flex-col relative border-x border-white/5">
              ...
              <div className="flex justify-between items-start">
                 <div>
                     <div className="flex items-center gap-2 mb-1">
                         <Database size={10} className={`text-emerald-500 ${!isStale ? 'animate-pulse' : ''}`} />
                         <span className={`text-[9px] font-bold uppercase tracking-[0.2em] ${isStale ? 'text-amber-500' : 'text-emerald-500'}`}>
                             {isStale ? 'OFFLINE ARCHIVE' : 'SECURE DATABASE'}
                         </span>
                     </div>
           ...
           <div className="flex-1 overflow-y-auto px-4 pb-32 hide-scrollbar relative z-10 pt-4">
               ...
               {/* Database Timestamp Footer */}
               {isStale && (
                   <div className="pb-4 pt-2 text-center">
                       <p className="text-[8px] text-slate-600 font-mono uppercase tracking-widest flex items-center justify-center gap-1">
                           <Clock size={8} /> Last Synced: {timeAgo}
                       </p>
                   </div>
               )}
           </div>
