...
const SkaterGame: React.FC<SkaterGameProps> = ({ onClose, isOverlay = false }) => {
  const { grantXp } = useAppStore(); // Connected to store for XP
    ...
    if (s.grounded || s.grinding) {
      ...
      triggerHaptic('light');
        ...
        if (s.airTapCount === 2) {
            ...
            s.dy = Math.min(s.dy, -8); // Controlled boost
        } else if (s.airTapCount === 3) {
            s.currentTrick = 'Kickflip';
            s.trickFrame = 0;
            playSound('skate_pop');
            triggerHaptic('heavy');
            s.dy = Math.min(s.dy, -6);
        }
    }
  };
  ...
  const gameOver = () => {
    ...
    // XP Grant Logic
    // Only grant XP for meaningful runs to prevent spam
    if (finalScore > 100) {
        const xpAmount = Math.min(500, Math.floor(finalScore / 5));
        grantXp(xpAmount, `Reflex Sim: ${finalScore} pts`);
    }
  };
    ...
    const update = (cvs: HTMLCanvasElement) => {
      ...
      if (!s.skater.grounded && !s.skater.grinding) {
        ...
        s.skater.y += s.skater.dy;
          ...
          s.skater.dy = 0; 
          // Constant sparks during grind (reduced frequency)
          if (s.skater.frameTick % 6 === 0) {
              for(let i=0; i<2; i++) {
                  s.particles.push({
      ...
      if (!lastOb || (cvs.width - (lastOb.x + lastOb.w) > minDistance)) {
        if (Math.random() < 0.65) {
          ...
          const choices = ['HYDRANT', 'BOX']; 

          if (dist > 500) { choices.push('RAIL', 'GUARD'); }
          if (dist > 1200) { choices.push('STAIRS', 'DRONE'); }
          if (dist > 2000) { choices.push('GAP'); }

          type = choices[Math.floor(Math.random() * choices.length)];

          if (type === 'BOX') { w = WORLD_PIXEL_SIZE * 40; h = WORLD_PIXEL_SIZE * 10; grindable = true; }
          else if (type === 'RAIL') { w = WORLD_PIXEL_SIZE * 60; h = WORLD_PIXEL_SIZE * 10; grindable = true; }
      ...
      for (let i = s.obstacles.length - 1; i >= 0; i--) {
        ...
        if (ob.type !== 'GAP') {
            ...
            if (colliding) {
                ...
                // 1. Maintain existing grind
                if (s.skater.grinding) {
                    s.skater.y = obstacleTop - skaterH;
                    s.skater.dy = 0;
                    isCurrentlyGrinding = true;
                }
                // 2. Enter new grind (Must be airborne and falling onto it)
                else if (ob.grindable && !s.skater.grounded && s.skater.dy >= 0 && playerBottom <= (obstacleTop + tolerance)) {
                    s.skater.y = obstacleTop - skaterH; // Snap
                    s.skater.dy = 0;
