...
import { Star, Send, Loader2, Play, X, Calendar, Clock, CheckCircle2, User as UserIcon, BadgeCheck, Zap, Filter, Bot, MessageSquare, ChevronRight, Terminal, BrainCircuit, Activity, Settings, Users, ChevronDown, Cpu, Lock } from 'lucide-react';
...
const MentorshipView: React.FC = () => {
  ...
  return (
    <div className="pb-32 pt-8 px-4 md:px-6 animate-view h-full flex flex-col relative overflow-y-auto hide-scrollbar bg-[#050505] font-mono">
      ...
      {/* --- AI COACH INTERFACE (Overlay Drawer Portal) --- */}
      {createPortal(
        <div 
            className={`
        ...
        >
            <div className={`flex-1 flex flex-col bg-[#0b0c10] rounded-2xl overflow-hidden border-2 border-emerald-500/30 shadow-[0_-10px_40px_rgba(0,0,0,0.8)] relative w-full max-w-2xl mx-auto ${activeTab === 'ai-coach' ? 'pointer-events-auto' : ''}`}>
                {/* Header */}
                <div className="bg-[#080a0f] p-4 border-b border-white/10 flex items-center justify-between shrink-0 cursor-pointer" onClick={() => setActiveTab('find')}>
                  ...
                  {filteredMentors.map(mentor => (
                      ...
                      >
                          ...
                          <div className="flex items-start gap-5 relative z-10">
                              {/* Avatar Block */}
                              <div className="relative w-20 h-20 rounded-md overflow-hidden bg-slate-900 border border-white/10 shadow-inner shrink-0 group-hover:border-indigo-500/50 transition-colors">
                              ...
                              <div className="flex-1 pt-1 min-w-0">
                                  <div className="flex items-center justify-between mb-1">
                                      <h3 className="text-xl font-black italic uppercase text-white leading-none tracking-tighter truncate group-hover:text-indigo-300 transition-colors font-sans">{mentor.name}</h3>
                                      <div className="flex items-center gap-1 bg-green-950/50 border border-green-500/20 px-1.5 py-0.5 rounded-sm">
                                  ...
                                  <div className="flex flex-wrap gap-1 mb-2.5">
                                      {mentor.disciplines.map(d => (
                                          <span key={d} className="text-[7px] font-black uppercase bg-[#151515] text-slate-300 px-2 py-0.5 rounded-sm tracking-wider border border-white/5 group-hover:border-white/10">
                                              {d}
                                          </span>
                                      ))}
                                      {mentor.badges.includes(MentorBadge.CERTIFIED) && (
                                          <span className="text-[7px] font-black uppercase bg-blue-900/30 text-blue-400 px-2 py-0.5 rounded-sm tracking-wider border border-blue-500/30 flex items-center gap-1">
                          ...
                          {/* Stats Grid */}
                          <div className="grid grid-cols-3 gap-px bg-white/5 border border-white/5 rounded-md overflow-hidden mt-1 relative z-10">
      ...
      {activeTab === 'apply' && (
          <div className="flex-1 bg-[#0b0c10] rounded-2xl border-2 border-slate-800 p-6 flex flex-col items-center justify-center text-center animate-view relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/20 to-black pointer-events-none" />
              
              {isEligibleForMentor ? (
                  <div className="relative z-10 space-y-4">
                      <div className="w-16 h-16 bg-indigo-600 rounded-md flex items-center justify-center mx-auto shadow-2xl shadow-indigo-600/30 border border-indigo-400">
                          <CheckCircle2 size={32} className="text-white" />
                      </div>
                      <h2 className="text-2xl font-black italic uppercase text-white tracking-tighter font-sans">Clearance Granted</h2>
                      ...
                      <button 
                        ...
                        className="bg-white text-black px-8 py-4 rounded-md text-xs font-black uppercase tracking-widest shadow-xl hover:bg-slate-200 transition-all active:scale-95 mt-4"
                      >
                          Start Application
                      </button>
                  </div>
                  ...
                  <div className="relative z-10 space-y-6">
                      ...
                      <div>
                          <h2 className="text-2xl font-black italic uppercase text-white tracking-tighter mb-2 font-sans">Access Denied</h2>
                          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Requirements not met</p>
                      </div>
                      
                      <div className="bg-black/40 rounded-lg p-4 text-left space-y-3 border border-white/5 w-full max-w-xs">
      ...
      {selectedMentor && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-view" onClick={closeProfile}>
            <div className="w-full max-w-sm bg-[#09090b] border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] relative flex flex-col overflow-hidden animate-pop max-h-[85vh]" onClick={(e) => e.stopPropagation()}>
                
                {/* Header Image Area */}
                <div className="relative h-80 w-full shrink-0">
                    {/* Background Pattern/Gradient */}
                    <div className="absolute inset-0 bg-gradient-to-b from-indigo-900/20 to-[#09090b]" />
                    <div className="absolute inset-0 opacity-20 bg-[radial-gradient(#4f46e5_1px,transparent_1px)] [background-size:16px_16px]" />
                    
                    {/* Close Button */}
                    <button 
                        onClick={closeProfile} 
                        className="absolute top-4 left-4 z-30 w-8 h-8 bg-black/20 backdrop-blur-md rounded-full text-white/70 border border-white/10 flex items-center justify-center hover:bg-white/10 hover:text-white transition-all"
                    >
                        <X size={16} />
                    </button>

                    {/* Status Tag */}
                    <div className="absolute top-4 right-4 z-30 flex items-center gap-1.5 bg-emerald-500/10 backdrop-blur-md border border-emerald-500/20 px-2.5 py-1 rounded-full">
                        <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_8px_#10b981]" />
                        <span className="text-[9px] font-bold uppercase text-emerald-400 tracking-wider">Online</span>
                    </div>
                    ...
                    {/* Overlay Info */}
                    <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#09090b] via-[#09090b]/80 to-transparent pt-20">
                        <h1 className="text-4xl font-black italic uppercase text-white tracking-tighter mb-2 leading-[0.9] font-sans drop-shadow-lg">
                            {selectedMentor.name}
                        </h1>
                        
                        <div className="flex items-center gap-3">
                            {selectedMentor.badges.includes(MentorBadge.CERTIFIED) && (
                                <span className="bg-blue-600 text-white text-[9px] font-black px-2 py-0.5 rounded flex items-center gap-1 uppercase tracking-wide shadow-lg shadow-blue-900/50">
                                    <BadgeCheck size={10} strokeWidth={3} /> Certified
                                </span>
                            )}
                            <span className="text-lg font-bold text-white/90 font-mono">
                                ₹{selectedMentor.rate}<span className="text-xs text-slate-500 font-sans font-bold">/hr</span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                {/* Scrollable Body */}
                <div className="flex-1 overflow-y-auto hide-scrollbar px-6 pb-6 bg-[#09090b] space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-2 p-1 bg-white/5 rounded-2xl border border-white/5">
                        <div className="flex flex-col items-center py-3">
                            <div className="flex items-center gap-1 text-yellow-400 mb-1">
                                ...
                                <span className="text-sm font-black">{selectedMentor.rating}</span>
                            </div>
                            <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest">Rating</span>
                        </div>
                        <div className="flex flex-col items-center py-3 border-x border-white/5">
                            <div className="flex items-center gap-1 text-indigo-400 mb-1">
                                <Users size={12} />
                                <span className="text-sm font-black">{selectedMentor.studentsTrained}</span>
                            </div>
                        ...
                        <div className="flex flex-col items-center py-3">
                            <div className="flex items-center gap-1 text-emerald-400 mb-1">
                                <Activity size={12} />
                                <span className="text-sm font-black">Lvl {selectedMentor.badges.includes(MentorBadge.EXPERT) ? '50' : '25'}</span>
                            </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-end border-b border-white/10 pb-2">
                            <h3 className="text-xs font-black uppercase text-white tracking-widest flex items-center gap-2">
                                <Calendar size={12} className="text-indigo-500" /> Session Booking
                            </h3>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {MOCK_SLOTS.map(slot => (
                                <button 
                                    ...
                                    className={`py-3 rounded-xl text-[10px] font-bold uppercase tracking-wide transition-all border ${
                                        selectedSlot === slot 
                                        ? 'bg-white text-black border-white shadow-lg scale-[1.02]' 
                                        : 'bg-[#121214] text-slate-400 border-white/5 hover:border-white/20 hover:text-white'
                                    }`}
                                >
                                    {slot}
                                </button>
                ...
                <div className="p-6 pt-0 bg-[#09090b]">
                     <button 
                        ...
                        className={`w-full h-14 rounded-2xl flex items-center justify-center gap-3 font-black uppercase tracking-[0.15em] text-xs transition-all shadow-xl relative overflow-hidden group ${selectedSlot ? 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-indigo-900/20' : 'bg-[#121214] text-slate-600 border border-white/5 cursor-not-allowed'}`}
                    >
                        {isBooking ? <><Loader2 className="animate-spin" size={16} /><span>Processing...</span></> : bookingSuccess ? <><CheckCircle2 size={16} className="text-white" /><span>Confirmed</span></> : <><Clock size={16} /><span>{selectedSlot ? 'Confirm Request' : 'Select Time'}</span></>}
                     </button>
                </div>
            </div>
        </div>
