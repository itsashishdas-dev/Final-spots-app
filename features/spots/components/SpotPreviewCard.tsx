...
import { MapPin, Navigation, Clock, Zap, X, Users, Droplets, Wind, Activity, ChevronRight, AlertTriangle, Plus, Swords, Calendar, Share2, EyeOff, Play, Video, Image as ImageIcon, MessageSquare, UserPlus, Mountain } from 'lucide-react';
import { triggerHaptic } from '../../../utils/haptics';
import { useAppStore } from '../../../store';
import { useLayoutMode } from '../../../hooks/useLayoutMode';
import { playSound } from '../../../utils/audio';
import { useSpotsData } from '../hooks/useSpots';
...
const SpotPreviewCard: React.FC<SpotPreviewCardProps> = ({ spot, sessions, challenges, crew, onClose, onCheckIn }) => {
  const { openModal, user, openChat, joinSession } = useAppStore();
  const { userLocation } = useSpotsData();
  ...
  const handleJoinSession = async (sessionId: string, e: React.MouseEvent) => {
      e.stopPropagation();
      triggerHaptic('success');
  ...
  const cardClasses = layoutMode === 'compact'
    ? 'w-full bg-[#050505] rounded-t-2xl shadow-2xl relative flex flex-col pointer-events-auto border-t-2 border-slate-700 animate-[enter-sheet_0.3s_cubic-bezier(0.16,1,0.3,1)_forwards] max-h-[85vh]'
    : 'w-full h-auto max-h-full bg-[#050505] rounded-2xl shadow-2xl relative flex flex-col pointer-events-auto border-2 border-slate-700 animate-[enter-panel-left_0.3s_cubic-bezier(0.16,1,0.3,1)_forwards]';
  ...
  return (
    <div className={containerClasses}>
        <div className={cardClasses}>
            ...
            <div className="absolute top-2 left-2 w-1.5 h-1.5 bg-slate-800 rounded-full z-50 border border-slate-900 shadow-sm" />
            <div className="absolute top-2 right-2 w-1.5 h-1.5 bg-slate-800 rounded-full z-50 border border-slate-900 shadow-sm" />

            <div className="relative w-full h-64 shrink-0 bg-black rounded-t-2xl overflow-hidden group border-b-2 border-slate-800">
                <div className="absolute top-4 left-4 z-30 pointer-events-auto">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-md backdrop-blur-md border shadow-lg ${status.border} ${status.bg}`}>
                ...
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 z-30 w-8 h-8 bg-black/60 backdrop-blur-md rounded-md text-white border border-white/20 hover:bg-white/20 active:scale-90 transition-all flex items-center justify-center shadow-lg pointer-events-auto"
                >
                    <X size={16} />
                </button>
                            ...
                            {item.type === 'video' ? (
                                <>
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                                        <div className="w-14 h-14 rounded-xl bg-white/10 backdrop-blur-sm flex items-center justify-center border border-white/30 shadow-2xl group-hover:scale-110 transition-transform cursor-pointer">
                                            <Play size={24} className="fill-white text-white ml-1" />
                                        </div>
                                    </div>
                                    <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
                                        <span className="inline-flex items-center gap-1.5 px-2 py-1 bg-red-500/20 backdrop-blur rounded text-[8px] font-bold uppercase text-red-100 border border-red-500/30 font-mono">
                                ...
                                <>
                                    ...
                                    <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
                                        <span className="inline-flex items-center gap-1.5 px-2 py-1 bg-white/10 backdrop-blur rounded text-[8px] font-bold uppercase text-white border border-white/10 font-mono">
                ...
                {mediaItems.length > 1 && (
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-30">
                        {mediaItems.map((_, i) => (
                            <div key={i} className="w-1.5 h-1.5 rounded-sm bg-white/80 shadow-sm" /> 
                        ))}
                    </div>
                )}
            </div>

            <div className="flex-1 overflow-y-auto hide-scrollbar bg-[#050505] relative z-20 -mt-6 rounded-t-2xl border-t-2 border-slate-700 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] font-mono">
                <div className="px-6 pt-8 pb-32 space-y-6">
                    
                    <div className="border-b border-white/10 pb-4">
                        <div className="flex justify-between items-start">
                            <h3 className="text-3xl font-black italic uppercase text-white tracking-tighter leading-[0.9] mb-1 font-sans">
                                {spot.name}
                            </h3>
                            {spot.privacy !== SpotPrivacy.PUBLIC && (
                                <div className="bg-red-900/20 border border-red-500/30 p-1.5 rounded-md">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {crew && (
                                <div className="flex items-center gap-1 bg-slate-900 border border-slate-700 rounded-sm px-1.5 py-0.5">
                    ...
                    <div className="grid grid-cols-2 border-2 border-slate-800 rounded-xl bg-[#0b0c10] relative">
                        ...
                        <div className="p-4 border-r border-b border-slate-800 flex flex-col gap-1">
                        ...
                        <div className="p-4 border-r border-slate-800 flex flex-col gap-1">
                        ...
                        <div className="p-4 flex flex-col gap-1">
                            ...
                            <span className="text-sm text-yellow-400 font-mono font-bold flex items-center gap-1">
                                {spot.rating.toFixed(1)} <span className="text-[8px] text-slate-600">/ 5.0</span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button 
                            ...
                            className="bg-[#111] border border-white/10 p-3 rounded-lg flex items-center gap-3 hover:bg-white/5 active:scale-95 transition-all group shadow-md"
                        >
                            <div className="w-8 h-8 rounded-md bg-indigo-500/20 text-indigo-400 flex items-center justify-center group-hover:text-white group-hover:bg-indigo-600 transition-colors border border-indigo-500/30">
                                <Calendar size={14} />
                            </div>
                        ...
                        <button 
                            ...
                            className="bg-[#111] border border-white/10 p-3 rounded-lg flex items-center gap-3 hover:bg-white/5 active:scale-95 transition-all group shadow-md"
                        >
                            <div className="w-8 h-8 rounded-md bg-red-500/20 text-red-400 flex items-center justify-center group-hover:text-white group-hover:bg-red-600 transition-colors border border-red-500/30">
                                <Swords size={14} />
                            </div>
                            <div className="text-left">
                    ...
                    {canInvite && spot.privacy !== SpotPrivacy.PUBLIC && (
                        <button 
                            ...
                            className="w-full bg-slate-900 border border-slate-700 border-dashed rounded-lg p-3 flex items-center justify-center gap-2 text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-white hover:border-slate-500 transition-all active:scale-95"
                        >
                    ...
                    <div>
                        ...
                        <div className="bg-[#0b0c10] border border-slate-800 rounded-lg overflow-hidden transition-all shadow-lg">
                            <button 
                            ...
                            {isLiveOpsExpanded && sessions.length > 0 && (
                                <div className="border-t border-slate-800 p-2 space-y-2 bg-[#080808]">
                                    {sessions.map(sess => {
                                        const isJoined = sess.participants.includes(user?.id || '');
                                        return (
                                            <div key={sess.id} className="bg-[#111] rounded-md p-3 flex justify-between items-center border border-white/5">
                                                <div className="flex items-center gap-3">
                                                    <img src={`https://api.dicebear.com/7.x/pixel-art/svg?seed=${sess.userId}`} className="w-8 h-8 rounded-md bg-slate-800" />
                                                    <div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {isJoined ? (
                                                        <button 
                                                            ...
                                                            className="p-2 text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 rounded-md transition-colors active:scale-95 border border-indigo-500/20"
                                                        >
                                                            <MessageSquare size={14} />
                                                        </button>
                                                    ) : (
                                                        <button 
                                                            onClick={(e) => handleJoinSession(sess.id, e)}
                                                            className="px-3 py-1.5 bg-white text-black rounded-md text-[8px] font-black uppercase tracking-wider hover:bg-slate-200 transition-colors shadow-lg active:scale-95"
                                                        >
                    ...
                    <div className="grid grid-cols-[1fr_1.5fr] gap-3 pt-2">
                        <button 
                            ...
                            className="py-4 bg-[#111] text-white border-2 border-slate-800 rounded-xl font-black uppercase text-[10px] tracking-[0.15em] flex items-center justify-center gap-2 hover:bg-[#1a1a1a] transition-all active:scale-95 group shadow-lg"
                        >
                        ...
                        <button 
                            ...
                            className="py-4 bg-indigo-600 text-white rounded-xl font-black uppercase text-[10px] tracking-[0.15em] flex items-center justify-center gap-2 hover:bg-indigo-500 transition-all active:scale-95 shadow-[0_0_20px_rgba(99,102,241,0.3)] group relative overflow-hidden border-2 border-indigo-500"
                        >
